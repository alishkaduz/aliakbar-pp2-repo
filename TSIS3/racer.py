...
def play_game(screen, username):
    ...
    image_player_orig     = pygame.image.load('assets/Player.png')
    image_enemy_orig     = pygame.image.load('assets/Enemy.png')
    coin_image       = pygame.image.load('assets/dollar.png').convert_alpha()

    scale_factor = 0.15  # Try 0.5 for even smaller, 0.8 for slightly smaller
    new_player_width = int(image_player_orig.get_width() * scale_factor)
    new_player_height = int(image_player_orig.get_height() * scale_factor)
    new_enemy_width = int(image_enemy_orig.get_width() * 0.25)
    new_enemy_height = int(image_enemy_orig.get_height() * 0.25)
    image_player = pygame.transform.scale(image_player_orig, (new_player_width, new_player_height))
    image_enemy = pygame.transform.scale(image_enemy_orig, (new_enemy_width, new_enemy_height))
